...
export default function AdminDashboard() {
  ...
  const [recentBookings, setRecentBookings] = useState([
    { id: "BK-9021", customer: "John Smith", service: "Home Cleaning", date: "Sept 04, 2026", status: "Confirmed", amount: "$120" },
    { id: "BK-9022", customer: "Aisha Khan", service: "Plumbing Repair", date: "Sept 04, 2026", status: "Pending", amount: "$85" },
    { id: "BK-9023", customer: "Sara Wilson", service: "AC Maintenance", date: "Sept 05, 2026", status: "Needs Info", amount: "$150" },
    { id: "BK-9024", customer: "Liam Brown", service: "Electrical Check", date: "Sept 05, 2026", status: "In Progress", amount: "$95" },
    { id: "BK-9025", customer: "Omar Farooq", service: "Deep Cleaning", date: "Sept 06, 2026", status: "Completed", amount: "$200" },
  ]);
  ...
  return (
    <div className="p-6 md:p-8 min-h-screen bg-[#030712] text-slate-100 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Link href="/admin/payments" className="bg-[#0b0f19] border border-slate-800/80 rounded-2xl p-5 hover:border-teal-500/50 transition cursor-pointer group shadow-lg">
          <div className="flex justify-between items-start">
            <span className="text-xs font-semibold text-slate-400">Total Revenue</span>
            <div className="w-9 h-9 rounded-xl bg-teal-500/10 border border-teal-500/20 flex items-center justify-center text-teal-400 group-hover:scale-110 transition">
              <DollarSign className="w-4 h-4" />
            </div>
          </div>
          <div className="mt-3">
            <h3 className="text-2xl font-extrabold text-white">$16,800</h3>
            <span className="text-[11px] font-bold text-emerald-400 mt-1 inline-block">↗ +15% vs last month</span>
          </div>
        </Link>

        <Link href="/admin/bookings" className="bg-[#0b0f19] border border-slate-800/80 rounded-2xl p-5 hover:border-blue-500/50 transition cursor-pointer group shadow-lg">
          <div className="flex justify-between items-start">
          ...
          <div className="mt-3">
            ...
            <span className="text-[11px] font-bold text-emerald-400 mt-1 inline-block">↗ +8.4% growth</span>
          </div>
        </Link>

        <Link href="/admin/services" className="bg-[#0b0f19] border border-slate-800/80 rounded-2xl p-5 hover:border-amber-500/50 transition cursor-pointer group shadow-lg">
          <div className="flex justify-between items-start">
          ...
          <div className="mt-3">
            ...
            <span className="text-[11px] font-bold text-teal-400 mt-1 inline-block">Ready for Booking</span>
          </div>
        </Link>

        <Link href="/admin/customers" className="bg-[#0b0f19] border border-slate-800/80 rounded-2xl p-5 hover:border-indigo-500/50 transition cursor-pointer group shadow-lg">
          <div className="flex justify-between items-start">
          ...
          <div className="mt-3">
            ...
            <span className="text-[11px] font-bold text-teal-400 mt-1 inline-block">High Customer Rating</span>
          </div>
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-[#0b0f19] border border-slate-800/80 rounded-2xl p-6 flex flex-col justify-between shadow-xl">
          <div>
            <h2 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Monthly Revenue Performance</h2>
            <p className="text-2xl font-extrabold text-white mt-1">$19,400 <span className="text-xs font-normal text-teal-400">Peak Sep 2026</span></p>
          </div>
          ...
          <div className="flex items-end justify-between h-44 mt-6 px-4 pb-2 border-b border-slate-800">
            {[
              ...
              <div key={idx} className="flex flex-col items-center gap-2 group">
                <div className={`w-10 ${bar.height} bg-teal-500/80 group-hover:bg-teal-400 rounded-t-lg transition-all duration-200 shadow-lg shadow-teal-500/10`} />
          ...
          <div>
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="flex gap-1 text-slate-400 text-xs">
                <button className="p-1 hover:text-white bg-slate-800/50 rounded-md"><ChevronLeft className="w-4 h-4" /></button>
                <button className="p-1 hover:text-white bg-slate-800/50 rounded-md"><ChevronRight className="w-4 h-4" /></button>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-[#0b0f19] border border-slate-800/80 rounded-2xl p-6 shadow-xl space-y-4">
          <div className="flex justify-between items-center">
            ...
            <Link href="/admin/bookings" className="text-xs font-bold text-teal-400 hover:text-teal-300 transition">
              View All →
            </Link>
          </div>
                  ...
                  <tr key={b.id} className="hover:bg-slate-800/30 transition">
                    ...
                    <td className="py-3.5 px-2">
                      <select
                        ...
                        className={`border rounded-lg text-[11px] font-bold px-2.5 py-1 focus:outline-none cursor-pointer ${getStatusBadgeClass(b.status)}`}
                      >
        ...
        <div className="bg-[#0b0f19] border border-slate-800/80 rounded-2xl p-6 shadow-xl space-y-4">
          ...
          <div className="space-y-3">
            {[
              { name: "Michael R.", review: "Excellent home cleaning service! Very professional staff.", time: "2 hours ago" },
              { name: "Fatima Z.", review: "Plumber arrived right on time and fixed the leak quickly.", time: "Yesterday" }
            ].map((rev, idx) => (
              <div key={idx} className="p-4 bg-[#0f1629] border border-slate-800/80 rounded-xl space-y-2">
                <div className="flex justify-between items-center">
                ...
                <span className="text-[10px] text-slate-500 block">{rev.time}</span>
              </div>
            ))}
          </div>
        </div>
